...
# --- SDK compatibility shims (must run before any Activity validation) ---
from sdk_compat import apply_product_info_compat

apply_product_info_compat()

# --- Outbound-HTTP diagnostics: capture Agent 365 tool (MCP) error bodies ---
from mcp_diag import apply_httpx_diagnostics

apply_httpx_diagnostics()
...
class GenericAgentHost:
    ...
    def _seen_activity(self, activity_id: str | None) -> bool:
        """Return True if this activity id was already processed (a redelivery).

        Records unseen ids and evicts entries older than the TTL (or once the
        buffer grows past its cap) so memory stays bounded.
        """
        if not activity_id:
            return False
        now_ts = time.monotonic()
        # Evict expired entries.
        while self._recent_activity_ids:
            oldest_id, oldest_ts = next(iter(self._recent_activity_ids.items()))
            if (now_ts - oldest_ts) > self._recent_activity_ttl or len(
                self._recent_activity_ids
            ) > self._recent_activity_max:
                self._recent_activity_ids.pop(oldest_id, None)
            else:
                break
        if activity_id in self._recent_activity_ids:
            return True
        self._recent_activity_ids[activity_id] = now_ts
        return False
    ...
    def _setup_handlers(self):
        ...
        async def on_message(context: TurnContext, _: TurnState):
            try:
                # Skip redelivered duplicates so a slow/failing turn doesn't spam
                # the user with repeated acknowledgements.
                if self._seen_activity(context.activity.id):
                    logger.info(
                        "↩️ Duplicate delivery of activity %s — skipping",
                        context.activity.id,
                    )
                    return
                ...
                from mcp_diag import format_exception_notice

                notice = format_exception_notice(
                    e, "message handling (host)", os.getenv("CONTAINER_APP_NAME")
                )
                await context.send_activity(f"Sorry, I encountered an error.\n{notice}")
