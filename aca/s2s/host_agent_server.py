...
_UI_AUDIENCE = os.getenv("UI_AUDIENCE", "")  # app id del blueprint (aud dei token v2)
_UI_ALLOWED_ORIGINS = [
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        ...
        if auth_configuration:
            ...
            async def jwt_with_health_bypass(request, handler):
                # Skip JWT validation for health, the SPA /chat endpoint (own token
                # validation), and CORS preflight, so container orchestrators and the
                # SPA can reach the agent without a Bot Framework token.
