...
_UI_AUDIENCE = os.getenv("UI_AUDIENCE", "")  # app id del blueprint OBO (aud dei token v2)
_UI_ALLOWED_ORIGINS = [
    o.strip() for o in os.getenv("UI_ALLOWED_ORIGINS", "").split(",") if o.strip()
]
_MCP_RESOURCE = "ea9ffc3e-8a23-4a7d-836d-234d7c7565c1"  # Agent 365 Tools (Mail MCP)
_OBO_CLIENT_ID = os.getenv("CONNECTIONS__SERVICE_CONNECTION__SETTINGS__CLIENTID", "")
_OBO_CLIENT_SECRET = os.getenv("CONNECTIONS__SERVICE_CONNECTION__SETTINGS__CLIENTSECRET", "")
_jwks_client = None
...
def _get_jwks_client():
    ...
    if _jwks_client is None:
        ...
        url = f"https://login.microsoftonline.com/{_UI_TENANT_ID}/discovery/v2.0/keys"
        _jwks_client = _jwtlib.PyJWKClient(url)
    return _jwks_client
...
def validate_user_token(token: str) -> dict:
    ...
    audiences = [a for a in (_UI_AUDIENCE, f"api://{_UI_AUDIENCE}") if _UI_AUDIENCE]
    decoded = _jwtlib.decode(
        token,
        signing_key.key,
        algorithms=["RS256"],
        audience=audiences if audiences else None,
        options={"verify_aud": bool(audiences), "verify_exp": True},
    )
    iss = str(decoded.get("iss", ""))
    if _UI_TENANT_ID and _UI_TENANT_ID not in iss:
...
def obo_exchange_for_mail(user_token: str) -> str:
    ...
    authority = f"https://login.microsoftonline.com/{_UI_TENANT_ID}"
    app = msal.ConfidentialClientApplication(
        _OBO_CLIENT_ID, authority=authority, client_credential=_OBO_CLIENT_SECRET
    )
    result = app.acquire_token_on_behalf_of(
        user_assertion=user_token, scopes=[f"{_MCP_RESOURCE}/.default"]
    )
...
class GenericAgentHost:
    ...
    def start_server(self, auth_configuration: AgentAuthConfiguration | None = None):
        ...
        async def chat(req: Request) -> Response:
            ...
            # Mail MCP token: either the incoming delegated token itself (SPA acquired a
            # token for the Mail MCP resource) or an OBO-exchanged token. Agentic apps
            # cannot use the OBO (jwt-bearer) grant (AADSTS82002), so 'direct' is default.
            if os.getenv("MAIL_TOKEN_MODE", "direct").lower() == "direct":
                mail_token = token
            else:
                try:
                    mail_token = obo_exchange_for_mail(token)
                except Exception as e:
        ...
        async def cors_mw(request, handler):
            ...
            allowed = origin and (
                _UI_ALLOWED_ORIGINS == ["*"] or origin in _UI_ALLOWED_ORIGINS
            )
        ...
        middlewares.append(cors_mw)
        if auth_configuration:

            @web_middleware
            async def jwt_with_health_bypass(request, handler):
                # Skip JWT for health, the SPA /chat endpoint (own validation) and CORS preflight.
                if request.path in ("/api/health", "/chat") or request.method == "OPTIONS":
        ...
        app.router.add_route("OPTIONS", "/chat", lambda _r: Response(status=204))
        ...
        try:
            run_app(app, host=os.environ.get("HOST", "localhost"), port=port, handle_signals=True)
